...
module.exports.initialize = (callback) => {
    connection = mysql.createConnection({host: "localhost", user: "bot_staging", database: "bot_staging"});
...
module.exports.addReminder = (discordID, time, content, seconds, callback) => {
    connection.query("INSERT INTO reminders (invoked_on, content, delay_seconds, user_id) SELECT ?, ?, ?, id FROM users WHERE discord_id = ?;", [time, content, seconds, discordID], (error, results, fields) => {
        handleError(error);
        callback();
    });
}
...
module.exports.allReminders = (callback) => {
    connection.query(
        "SELECT reminders.invoked_on, reminders.content, reminders.delay_seconds, users.discord_id FROM reminders INNER JOIN users ON reminders.user_id = users.id;", (error, results, fields) => {
        handleError(error);
        callback(results);
    });
}

...
module.exports.addReminder = (discordID, content, seconds, callback) => {
    connection.query("INSERT INTO reminders (content, seconds, user_id) SELECT ?, ?, id FROM users WHERE discord_id = ?;", [content, seconds, discordID], (error, results, fields) => {
        handleError(error);
        callback();
    });
}
...
module.exports.allReminders = (callback) => {
    connection.query(
        "SELECT reminders.content, reminders.seconds, users.nickname FROM reminders INNER JOIN users ON quotes.user_id = users.id;", (error, results, fields) => {
        handleError(error);
        callback(results);
    });
}

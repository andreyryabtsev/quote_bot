...
module.exports.recentLogs = (discordID, count, callback) => {
    connection.query(
        "SELECT users.nickname, logs.created_at FROM logs INNER JOIN users ON logs.user_id = users.id WHERE users.discord_id = ? ORDER BY CAST(logs.created_at AS UNSIGNED) LIMIT ?;",
